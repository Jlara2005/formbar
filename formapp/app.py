...
if ONRPi:
    ...
    pixels = neopixel.NeoPixel(
        board.D21, MAXPIX, brightness=1.0, auto_write=False)
else:
...
words = json.loads(open(os.path.dirname(
    os.path.abspath(__file__)) + "/data/words.json").read())
...
def changeMode(newMode='', direction='next'):
    ...
    socket_.emit('modeChanged', {
                 'mode': sD.settings['barmode']}, namespace=apinamespace)
...
def showString(toShow, startLocation=0, fg=colors['fg'], bg=colors['bg']):
    for i, letter in enumerate(toShow.lower()):
        printLetter(letter, (i * (8 * 6)) +
                    ((startLocation * 48) + BARPIX), fg, bg)
    if ONRPi:
        pixels.show()
...
def abcdBar():
    ...
    for index, result in enumerate(results):
        ...
        pixRange = range((chunkLength * index),
    ...
    if sD.settings['captions']:
        clearString()
        showString("ABCD " + str(complete) + "/" +
                   str(sD.settings['numStudents']))
    if ONRPi:
        pixels.show()
...
def tutdBar():
    ...
    if upCount >= sD.settings['numStudents']:
        ...
        for i, pix in enumerate(range(0, BARPIX)):
            pixels[pix] = blend(range(0, BARPIX), i,
            ...
            if sD.settings['barmode'] == 'abcd':
                if vote in ["a", "b", "c", "d"]:
                    if sD.studentDict[request.remote_addr]['letter'] != vote:
                        ...
                        abcdBar()
                        socket_.emit('letter', {
                                     'name': sD.studentDict[request.remote_addr]['name'], 'letter': vote}, namespace=apinamespace)
                    ...
                    if sD.studentDict[request.remote_addr]['letter']:
                        ...
                        abcdBar()
                        socket_.emit('letter', {
                                     'name': sD.studentDict[request.remote_addr]['name'], 'letter': ''}, namespace=apinamespace)
...
def endpoint_addfighteropponent():
    ...
    # Set "opponent" of object to arg "name"
    sD.fighter['match' + code]['opponent'] = name
            ...
            if bgm_file in bgm.bgm:
                if time.time() - sD.bgm['lastTime'] >= 60 or sD.studentDict[request.remote_addr]['perms'] <= sD.settings['perms']['mod']:
                    ...
                    try:
                        ...
                        logFile(
                            " [warning] ", "Could not convert volume to float. Setting to default.")
                        bgm_volume = 0.5
...
def endpoint_break():
    ...
    if loginResult:
        ...
        name = request.args.get(
            'name') or sD.studentDict[request.remote_addr]['name'].strip()
        if request.args.get('action') == 'request':
            ...
            if sD.studentDict[request.remote_addr]['help']['type']:
                ...
                socket_.emit(
                    'help', sD.studentDict[request.remote_addr]['help'], namespace=apinamespace)
                ...
                for user in userFound:
                    if username in user:
                        ...
                        if oldPassword == cipher.decrypt(user[2]).decode():
                            passwordCrypt = cipher.encrypt(
                                newPassword.encode())
                            db = sqlite3.connect(os.path.dirname(
                                os.path.abspath(__file__)) + '/data/database.db')
                            dbcmd = db.cursor()
                            dbcmd.execute("UPDATE users SET password=:pw WHERE username=:uname", {
                                          "uname": username, "pw": passwordCrypt})
                            db.commit()
                            db.close()
    ...
    if loginResult:
                ...
                if isinstance(argVal, bool):
                    if arg in sD.settings:
                        ...
                        db = sqlite3.connect(os.path.dirname(
                            os.path.abspath(__file__)) + '/data/database.db')
                        dbcmd = db.cursor()
                        dbcmd.execute("UPDATE settings SET " +
                    ...
                    try:
                        ...
                        if arg in sD.settings['perms']:
                            if argInt > 4 or argInt < 0:
                                ...
                                dbcmd = db.cursor()
                                dbcmd.execute(
                                    "UPDATE settings SET " + arg + "Perm=:value", {"value": argInt})
                                db.commit()
                                db.close()
                    except Exception as e:
                        logFile("Error", str(e))
        ...
        if request.args.get('students'):
            ...
            if sD.settings['numStudents'] == 0:
                ...
                resString += 'Set <i>numStudents</i> to: ' + \
                    str(sD.settings['numStudents'])
                    
        if request.method == 'POST':
                 if request.form['barmode']:
                    if request.form['barmode'] in sD.settings['modes']:
                        sD.settings['barmode'] = request.form['barmode']
    ...
    if loginResult:
        ...
        if request.method == 'POST':
            ...
            if sD.settings['barmode'] == 'text':
                ...
                textBar()
                socket_.emit('essay', {
                             'name': sD.studentDict[request.remote_addr]['name'], 'essay': essay}, namespace=apinamespace)
...
def endpoint_games_2048():
    ...
    if loginResult:
        ...
        db = sqlite3.connect(os.path.dirname(
            os.path.abspath(__file__)) + '/data/database.db')
        dbcmd = db.cursor()
        highScore = dbcmd.execute(
            "SELECT * FROM scores WHERE username=:uname AND game='2048' ORDER BY score DESC", {"uname": username}).fetchone()
        db.close()
        if highScore:
...
def endpoint_games_bitshifter():
    ...
    if loginResult:
        ...
        dbcmd = db.cursor()
        highScore = dbcmd.execute(
            "SELECT * FROM scores WHERE username=:uname AND game='bitshifter' ORDER BY score DESC", {"uname": username}).fetchone()
        db.close()
        if highScore:
            highScore = highScore[3]
        else:
...
def endpoint_games_hangman():
    ...
    if loginResult:
        ...
        dbcmd = db.cursor()
        highScore = dbcmd.execute(
            "SELECT * FROM scores WHERE username=:uname AND game='hangman' ORDER BY score DESC", {"uname": username}).fetchone()
        db.close()
        if highScore:
            highScore = highScore[3]
        else:
...
def endpoint_games_minesweeper():
    ...
    if loginResult:
        ...
        db = sqlite3.connect(os.path.dirname(
            os.path.abspath(__file__)) + '/data/database.db')
        dbcmd = db.cursor()
        bestTime = dbcmd.execute(
            "SELECT * FROM scores WHERE username=:uname AND game='minesweeper' ORDER BY score ASC", {"uname": username}).fetchone()
        db.close()
        if bestTime:
            bestTime = bestTime[3]
        else:
...
def endpoint_games_speedtype():
    ...
    if loginResult:
        ...
        dbcmd = db.cursor()
        highScore = dbcmd.execute(
            "SELECT * FROM scores WHERE username=:uname AND game='speedtype' ORDER BY score DESC", {"uname": username}).fetchone()
        db.close()
        if highScore:
            highScore = highScore[3]
        else:
...
def endpoint_games_ttt():
    ...
    if loginResult:
        ...
        db = sqlite3.connect(os.path.dirname(
            os.path.abspath(__file__)) + '/data/database.db')
        dbcmd = db.cursor()
        wins = dbcmd.execute("SELECT * FROM scores WHERE username=:uname AND game='ttt' ORDER BY score DESC", {
                             "uname": sD.studentDict[request.remote_addr]['name']}).fetchone()
        if wins:
            wins = wins[3]
        else:
            wins = 0
        ...
        for student in sD.studentDict:
            if sD.studentDict[student]['name'] == opponent:
                sD.ttt.append(sessions.TTTGame(
                    [sD.studentDict[request.remote_addr]['name'], opponent]))
...
def endpoint_games_wordle():
    ...
    if loginResult:
        ...
        dbcmd = db.cursor()
        highScore = dbcmd.execute(
            "SELECT * FROM scores WHERE username=:uname AND game='wordle' ORDER BY score DESC", {"uname": username}).fetchone()
        db.close()
        if highScore:
            highScore = highScore[3]
        else:
...
def endpoint_leaderboards():
    ...
    if loginResult:
        ...
        dbcmd = db.cursor()
        data = dbcmd.execute(
            "SELECT * FROM scores ORDER BY score DESC").fetchall()
        db.close()
    ...
    if loginResult:
        ...
        if request.method == 'POST':
            if not request.files['file']:
                ...
                f.save(os.path.join('lessondata',
                       secure_filename(f.filename.strip(' '))))
                if request.args.get('advanced'):
                    advanced = '?advanced=true'
                else:
                    advanced = ''
            ...
            if not sD.lesson:
                ...
                resString += '<h2>Current Step: ' + \
                    str(sD.lesson.steps[sD.currentStep]['Prompt']) + '</h2>'
                resString += '<table>'
                # Agenda
...
def endpoint_login():
    ...
    if remote in banList:
        ...
        if request.method == 'POST':
            ...
            password = request.form['password']
            passwordCrypt = cipher.encrypt(
                password.encode())  # Required to be bytes?
...
def endpoint_profile():
    ...
    if loginResult:
        ...
        name = request.args.get(
            'user') or sD.studentDict[request.remote_addr]['name']
        userFound = False
        for x in sD.studentDict:
...
def endpoint_quiz():
    ...
    if loginResult:
        ...
        if request.method == 'POST':
            messageOut = packMSG(
                'alert', 'all', 'server', 'The teacher started a quiz.<br><button onclick="window.location=\"/quiz\"">Open quiz</button>')
            ...
            for i, answer in enumerate(request.form):
                ...
                if sD.activeQuiz['keys'][i] == int(request.form[answer]):
                    ...
                    sD.studentDict[request.remote_addr]['quizRes'].append(
                        False)
    ...
    if loginResult:
        ...
        try:
            ...
            if game and score:
                ...
                dbcmd = db.cursor()
                dbcmd.execute(
                    "INSERT INTO scores (game, username, score) VALUES (?, ?, ?)", (game, username, score))
                db.commit()
                db.close()
    ...
    if loginResult:
        ...
        if sD.activePhrase:
            if hex2dec(fgColor) and hex2dec(bgColor):
                clearString()
                showString(sD.activePhrase, 0, hex2dec(
                    fgColor), hex2dec(bgColor))
            else:
                clearString()
                showString(sD.activePhrase)
                if ONRPi:
    ...
    if loginResult:
        ...
        if type == 'fadein':
            ...
            if not hex2dec(hex):
                ...
                for i, pix in enumerate(pixRange):
                    pixels[pix] = blend(
                        pixRange, i, hex2dec(hex), hex2dec(hex2))
        elif type == 'color':
            for i, pix in enumerate(pixRange):
        ...
        if request.method == 'POST':
            if request.form['page'] == 'standard' or request.form['page'] == 'advanced' or request.form['page'] == 'quickpanel':
                ...
                db = sqlite3.connect(os.path.dirname(
                    os.path.abspath(__file__)) + '/data/database.db')
                dbcmd = db.cursor()
                dbcmd.execute("UPDATE users SET preferredHomepage=:page WHERE username=:uname", {
                              "uname": sD.studentDict[request.remote_addr]['name'], "page": request.form['page']})
                db.commit()
                db.close()
            else:
...
# Start a poll
@app.route('/startpoll', methods = ['POST', 'GET'])
...
@app.route('/tutd', methods = ['POST', 'GET'])
def endpoint_tutd():
    loginResult = loginCheck(request.remote_addr)
    if loginResult:
        return loginResult
            ...
            if sD.settings['barmode'] == 'tutd':
                ...
                if thumb in ['up', 'down', 'wiggle']:
                    if sD.studentDict[request.remote_addr]['thumb'] != thumb:
                        ...
                        tutdBar()
                        socket_.emit('thumb', {
                                     'name': sD.studentDict[request.remote_addr]['name'], 'thumb': thumb}, namespace=apinamespace)
                    ...
                    if sD.studentDict[request.remote_addr]['thumb']:
                        ...
                        tutdBar()
                        socket_.emit('thumb', {
                                     'name': sD.studentDict[request.remote_addr]['name'], 'thumb': ''}, namespace=apinamespace)
                        return render_template("message.html", message="I won\'t mention it if you don\'t")
                    else:
                        return render_template("message.html", message="You don't have an answer to erase.")
                else:
    ...
    if loginResult:
        ...
        try:
            ...
            username = request.args.get(
                "name") or sD.studentDict[request.remote_addr]['name']
            if field and value:
                db = sqlite3.connect(os.path.dirname(
                    os.path.abspath(__file__)) + '/data/database.db')
                dbcmd = db.cursor()
                dbcmd.execute("UPDATE users SET " + field +
                              "=:value WHERE username=:uname", {"uname": username, "value": value})
                db.commit()
                db.close()
        ...
        if request.args.get('name'):
            ...
            if action == 'delete':
                db = sqlite3.connect(os.path.dirname(
                    os.path.abspath(__file__)) + '/data/database.db')
                dbcmd = db.cursor()
                dbcmd.execute("DELETE FROM users WHERE username=:uname", {
            ...
            if action == 'changePw':
                ...
                if password:
                    ...
                    dbcmd = db.cursor()
                    dbcmd.execute("UPDATE users SET password=:pw WHERE username=:uname", {
                                  "uname": request.args.get('name'), "pw": passwordCrypt})
                    db.commit()
                    db.close()
        ...
        if user:
            if action == 'removeTicket':
                ...
                if request.args.get('acceptBreak'):
                    ...
                    # Get the student's current permissions so they can be restored later
                    sD.studentDict[student]['oldPerms'] = sD.studentDict[student]['perms']
            ...
            if action == 'kick':
                if user in sD.studentDict:
                    logFile(
                        "Info", sD.studentDict[request.remote_addr]['name'] + " was removed by the teacher.")
                    socket_.emit('alert', json.dumps(packMSG(
                        'all', 'server', sD.studentDict[request.remote_addr]['name'] + " was removed by the teacher...")), namespace=chatnamespace)
                    del sD.studentDict[user]
                    playSFX('sfx_laser01')
...
def endpoint_wawd():
    ...
    if sD.wawdLink:
        if sD.wawdLink[0] == "/":
            ...
            content += '<h2>External Resource</h2><a href="' + \
                str(sD.wawdLink) + '">' + str(sD.wawdLink) + '</a>'
    if not content:
...
def connect():
    try:
        if request.remote_addr in sD.studentDict:
            ...
            logFile("Info", sD.studentDict[request.remote_addr]['name'] +
                    " connected and was given id \"" + request.sid + "\"")
            emit('userlist', json.dumps(
    ...
    try:
        if request.remote_addr in sD.studentDict:
            if 'sid' in sD.studentDict[request.remote_addr]:
                ...
                logFile(
                    "Info", sD.studentDict[request.remote_addr]['name'] + " disconnected")
                emit('userlist', json.dumps(
                    packMSG('all', 'server', chatUsers())), broadcast=True)
    except Exception as e:
        logFile("Error", "Error finding user in list: " + str(e))
...
def message(message):
    try:
        ...
        if sD.studentDict[request.remote_addr]['perms'] > sD.settings['perms']['say']:
            messageOut = packMSG('alert', sD.studentDict[client['address'][0]]['name'],
                                 'server', "You do not have permission to send text messages.")
            ...
            dbcmd.execute("INSERT INTO messages ('from', 'to', 'time', 'content') VALUES (?, ?, ?, ?)",
                          (message['from'], message['to'], now, contentCrypt))
            db.commit()
            db.close()
            # Check recipients here
            if message['to'] == 'all':
                messageOut = packMSG(
                    'all', message['from'], message['content'], now)
                ...
                for student in sD.studentDict:
                    if sD.studentDict[student]['name'] == message['to'] or sD.studentDict[student]['name'] == message['from']:
                        messageOut = packMSG(
                            message['to'], message['from'], message['content'], now)
                        logFile("Chat", messageOut)
                        emit('message', json.dumps(messageOut),
                             to=sD.studentDict[student]['sid'])
                        break
            logFile("Info", message['from'] + " said to " +
...
def edit(timeSent, newContent):
    try:
        ...
        contentCrypt = cipher.encrypt(content.encode())
        dbcmd.execute("UPDATE messages SET content=:content WHERE time=" +
                      str(timeSent), {"content": contentCrypt})
        dbcmd.execute(
            "UPDATE messages SET edited=1 WHERE time=" + str(timeSent))
        db.commit()
        db.close()
...
def delete(timeSent):
    try:
        ...
        dbcmd.execute("UPDATE messages SET content=:content WHERE time=" +
                      str(timeSent), {"content": contentCrypt})
        dbcmd.execute(
            "UPDATE messages SET deleted=1 WHERE time=" + str(timeSent))
        db.commit()
        db.close()
...
def message(message):
    try:
        emit('userlist', json.dumps(packMSG(
            'userlist', sD.studentDict[request.remote_addr]['name'], 'server', chatUsers())), broadcast=True)
    except Exception as e:
        logFile("Error", str(e))
...
def message(message):
    try:
        emit('alert', client, json.dumps(packMSG(
            'alert', sD.studentDict[request.remote_addr]['name'], 'server', 'Only the server can send alerts!')))
    except Exception as e:
        logFile("Error", str(e))
